...
def get_bsdd_created() -> pd.DataFrame:
    ...
    df_bsdd_query = pd.read_sql_query(
        sqlalchemy.text(
            ...
            f"CAST((CAST(now() AS timestamp) + (INTERVAL '-{str(timeconf.time_delta_m)} month'))"
            "AS timestamp))"
...
df_bsdd_created = df_bsdd_created.loc[
    (df_bsdd_created["createdAt"] < timeconf.today)
    & (df_bsdd_created["createdAt"] >= timeconf.date_n_days_ago)
]
...
df_bsdd_processed = df_bsdd_processed.loc[
    (df_bsdd_processed["processedAt"] < timeconf.today)
...
df_company_user_created = df_company_user_created.loc[
    (timeconf.today > df_company_user_created["createdAt"])
    & (df_company_user_created["createdAt"] >= timeconf.date_n_days_ago)
]
